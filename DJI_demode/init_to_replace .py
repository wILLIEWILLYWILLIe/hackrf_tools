...
class hackrf_transfer1(Structure):
        _fields_ = [("device", p_hackrf_device),
                ("buffer", POINTER(c_byte)),
                ("buffer_length", c_int),
                ("valid_length", c_int),
...
_callback = CFUNCTYPE(c_int, POINTER(hackrf_transfer))
_callback1 = CFUNCTYPE(c_int, POINTER(hackrf_transfer1))
...
# libhackrf.hackrf_start_rx1.restype = c_int
# libhackrf.hackrf_start_rx1.argtypes = [p_hackrf_device, _callback1, c_void_p]
...
def read_samples_cb(hackrf_transfer):
    ...
    # print("len(values) = ",len(bytearray(values)))
    # print("len(bd) = ",len(this_hackrf.buffer))

    return 0

def read_samples_cb1(hackrf_transfer1):

    # let's access the contents
    c = hackrf_transfer1.contents
    # print(c)
    # c.device is an int representing the pointer to the hackrf device
    # we can get the pointer with p_hackrf_device(c.device)
    this_hackrf = _hackrf_dict[c.device]
...
rs_callback1 = _callback1(read_samples_cb1)
...
class HackRF(object):
    ...
    def read_samples(self,num_samples=131072,sleep_time=0.1):
        ...
        while self.still_sampling:
            ...
            sleep_count += 1
            if sleep_count >= 100:
                raise IOError("hackrf sleep to death...")
    ...
    # def read_samples_tmp(self,num_samples=131072,sleep_time=0.1):
    #     num_bytes = 2*num_samples
    #     self.num_bytes = int(num_bytes)
    #     # self.buffer = bytearray(self.num_bytes)
    #     self.buffer = bytearray()
    #     # number = self.get_serial_no()
    ...
    # def read_samples_tmp1(self,num_samples=131072,sleep_time=0.1):
    #     num_bytes = 2*num_samples
    #     self.num_bytes = int(num_bytes)
    #     # self.buffer = bytearray(self.num_bytes)
    #     self.buffer = bytearray()
    ...
    #     #print("before start")
    #     self.still_sampling = False
    #     result = libhackrf.hackrf_start_rx1(self.dev_p, rs_callback1, None)
    #     #print(result)
    #     if result != 0:
    #         raise IOError("Error in hackrf_start_rx")
...
if __name__ == '__main__':
    ...
    hackrf_center_freq = 5.7765e9
    hackrf_sample_rate = 15.36e6  # 2 MHz
    hackrf_duration = 0.35       # 0.01 秒
    hackrf_vga_gain = 36
    hackrf_lna_gain = 40

    hackrf = HackRF()
    #self.hackrf.disable_amp()
    hackrf.set_lna_gain(hackrf_lna_gain)
    hackrf.set_vga_gain(hackrf_vga_gain)
    hackrf.set_freq(hackrf_center_freq)
    hackrf.set_sample_rate(hackrf_sample_rate)
    count = 1
    while 1:
        print(f"count {count}")
        
        samples = hackrf.read_samples(hackrf_sample_rate*hackrf_duration)
        #subprocess.run(["hackrf_transfer", "-s", "8000000", "-f", "433918000", "-r", "myrec", "-b", "8000000"])        
        count = count + 1
        time.sleep(0.5)
    #samples = ((samples+1+1j)*256/2.00784314)
